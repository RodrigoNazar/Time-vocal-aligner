...
def read(file, readmarkers=False, readmarkerlabels=False, readmarkerslist=False, readloops=False, readpitch=False, normalized=False, forcestereo=False, log=False):
    ...
    * The data is returned as a numpy array with a
    ...
    #_cue = []
    #_cuelabels = []
    _markersdict = collections.defaultdict(
        lambda: {'position': -1, 'label': ''})
    loops = []
    pitch = 0.0
    while (fid.tell() < fsize):
        ...
        if chunk_id == b'fmt ':
            ...
            for c in range(numcue):
                ...
                _markersdict[idx]['position'] = position
        ...
        # see http://www.pjb.com.au/midi/sfspec21.html#i5
            ...
            label = fid.read(size-4).rstrip(bytes('\x00', 'UTF-8'))
            # _cuelabels.append(label)
            # needed to match labels and markers
            _markersdict[idx]['label'] = label
            ...
            for i in range(numsampleloops):
                str1 = fid.read(24)
                cuepointid, datatype, start, end, fraction, playcount = struct.unpack(
                    '<iiiiii', str1)
                loops.append([start, end])
        else:
            if log:
                warnings.warn("Chunk " + str(chunk_id) +
                              " skipped", WavFileWarning)
            _skip_unknown_chunk(fid)
    fid.close()
